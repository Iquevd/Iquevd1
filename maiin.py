tabla_periodica={ 
    'H':{
        'Grupo':'No metal',
        'Nombre':'Hidrogeno',
        'Número Atómico':1 ,
        'Peso Atómico':1.00797,
        'Estado de Oxidación':'+1' ,
        'Densidad(g/cm3)':0.071
    },
    'He':{
        'Grupo':'Gas noble',
        'Nombre':'Helio',
        'Número Atómico':2,
        'Peso Atómico':4.0026,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':0.126
    },
    'Li':{
        'Grupo':'Alcalino',
        'Nombre':'Litio',
        'Número Atómico':3,
        'Peso Atómico':6.941,
        'Estado de Oxidación':'+1',
        'Densidad(g/cm3)':0.53
    },
    'Be':{
        'Grupo':'Alcalinoterreo',
        'Nombre':'Berilio',
        'Número Atómico':4,
        'Peso Atómico':9.0122,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':1.85
    },
    'B':{
        'Grupo':'Semimetal',
        'Nombre':'Boro',
        'Número Atómico':5,
        'Peso Atómico':10.811 ,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':2.34
    },
    'C':{
        'Grupo':'No metal',
        'Nombre':'Carbono',
        'Número Atómico':6,
        'Peso Atómico':12.01115,
        'Estado de Oxidación':'+4',
        'Densidad(g/cm3)':2.26
    },
    'N':{
        'Grupo':'No metal',
        'Nombre':'Nitrogeno',
        'Número Atómico':7,
        'Peso Atómico':14.0067,
        'Estado de Oxidación':'-3',
        'Densidad(g/cm3)':0.81
    },
    'O':{
        'Grupo':'No metal',
        'Nombre':'Oxigeno',
        'Número Atómico':8,
        'Peso Atómico':15.9994,
        'Estado de Oxidación':'-2',
        'Densidad(g/cm3)':1.429
    },
    'F':{
        'Grupo':'No metal',
        'Nombre':'Fluor',
        'Número Atómico':9,
        'Peso Atómico':18.9984,
        'Estado de Oxidación':'-1',
        'Densidad(g/cm3)':1.11
    },
    'Ne':{
        'Grupo':'Gas noble',
        'Nombre':'Neon',
        'Número Atómico':10,
        'Peso Atómico':20.179,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':1.20
    },
    'Na':{
        'Grupo':'Alcalino',
        'Nombre':'Sodio',
        'Número Atómico':11,
        'Peso Atómico':22.9898,
        'Estado de Oxidación':'+1',
        'Densidad(g/cm3)':0.97
    },
    'Mg':{
        'Grupo':'Alcalinoterreo',
        'Nombre':'Magnesio',
        'Número Atómico':12,
        'Peso Atómico':24.305,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':1.74
    },
    'Al':{
        'Grupo':'Metal',
        'Nombre':'Aluminio',
        'Número Atómico':13,
        'Peso Atómico':26.9815,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':2.70,
    },
    'Si':{
        'Grupo':'Semimetal',
        'Nombre':'Silicio',
        'Número Atómico': 14 ,
        'Peso Atómico':28.086,
        'Estado de Oxidación':'+4',
        'Densidad(g/cm3)':2.33,
    },
    'P':{
        'Grupo':'No metal',
        'Nombre':'Fosforo',
        'Número Atómico':15,
        'Peso Atómico':30.9738,
        'Estado de Oxidación':'+5',
        'Densidad(g/cm3)':1.82,
    },
    'S':{
        'Grupo':'No metal',
        'Nombre':'Azufre',
        'Número Atómico':16,
        'Peso Atómico':32.064,
        'Estado de Oxidación':'-2',
        'Densidad(g/cm3)':2.07,
    },
    'Cl':{
        'Grupo':'No metal',
        'Nombre':'Cloro',
        'Número Atómico':17 ,
        'Peso Atómico':35.453,
        'Estado de Oxidación':'-1',
        'Densidad(g/cm3)':1.56,
    },
    'Ar':{
        'Grupo':'Gas noble',
        'Nombre':'Argon',
        'Número Atómico':18,
        'Peso Atómico':39.948,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':1.40,
    },
    'K':{
        'Grupo':'Alcalino',
        'Nombre':'Potasio',
        'Número Atómico':19,
        'Peso Atómico':39.098,
        'Estado de Oxidación':'+1',
        'Densidad(g/cm3)':0.97,
    },
    'Ca':{
        'Grupo':'Alcalinoterreo',
        'Nombre':'Calcio',
        'Número Atómico':20 ,
        'Peso Atómico':40.08,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':1.55,
    },
    'Sc':{
        'Grupo':'Metal',
        'Nombre':'Escandio',
        'Número Atómico':21 ,
        'Peso Atómico':44.956,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':3.00,
    },
    'Ti':{
        'Grupo':'Metal',
        'Nombre':'Titanio',
        'Número Atómico':22 ,
        'Peso Atómico':47.867,
        'Estado de Oxidación':'+4',
        'Densidad(g/cm3)':4.51,
    },
    'V':{
        'Grupo':'Metal',
        'Nombre':'Vanadio',
        'Número Atómico':23,
        'Peso Atómico':50.942,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':4.51,
    },
    'Cr':{
        'Grupo':'Metal',
        'Nombre':'Cromo',
        'Número Atómico':24,
        'Peso Atómico':51.996,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':7.19,
    },
    'Mn':{
        'Grupo':'Metal',
        'Nombre':'Manganeso',
        'Número Atómico':25,
        'Peso Atómico':54.938,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':7.43,
    },
    'Fe':{
        'Grupo':'Metal',
        'Nombre':'Hierro',
        'Número Atómico':26,
        'Peso Atómico':55.847,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':7.86,
    },
    'Co':{
        'Grupo':'Metal',
        'Nombre':'Cobalto',
        'Número Atómico':27,
        'Peso Atómico':58.93,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':8.9,
    },
    'Ni':{
        'Grupo':'Metal',
        'Nombre':'Niquel',
        'Número Atómico':28,
        'Peso Atómico':58.71,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':8.9,
    },
    'Cu':{
        'Grupo':'Metal',
        'Nombre':'Cobre',
        'Número Atómico':29,
        'Peso Atómico':63.54,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':8.96,
    },
    'Zn':{
        'Grupo':'Metal',
        'Nombre':'Zinc',
        'Número Atómico':30,
        'Peso Atómico':65.37,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':7.14,
    },
    'Ga':{
        'Grupo':'Metal',
        'Nombre':'Galio',
        'Número Atómico':31,
        'Peso Atómico':69.72,
        'Estado de Oxidación':'+3',
        'Densidad(g/cm3)':5.91,
    },
    'Ge':{
        'Grupo':'Semimetal',
        'Nombre':'Germanio',
        'Número Atómico':32,
        'Peso Atómico':72.59,
        'Estado de Oxidación':'+4',
        'Densidad(g/cm3)':5.32,
    },
    'As':{
        'Grupo':'Semimetal',
        'Nombre':'Arsenico',
        'Número Atómico':33,
        'Peso Atómico':74.922,
        'Estado de Oxidación':'+5',
        'Densidad(g/cm3)':5.72,
    },
    'Se':{
        'Grupo':'No metal',
        'Nombre':'Selenio',
        'Número Atómico':34,
        'Peso Atómico':78.96,
        'Estado de Oxidación':'-2',
        'Densidad(g/cm3)':4.79,
    },
    'Br':{
        'Grupo':'No metal',
        'Nombre':'Bromo',
        'Número Atómico':35,
        'Peso Atómico':79.909,
        'Estado de Oxidación':'-1',
        'Densidad(g/cm3)':3.12,
    },
    'Kr':{
        'Grupo':'Gas noble',
        'Nombre':'Krypton',
        'Número Atómico':36,
        'Peso Atómico':83.80,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':2.06,
    },
    'Rb':{
        'Grupo':'Alcalino',
        'Nombre':'Rubidio',
        'Número Atómico':37 ,
        'Peso Atómico':85.47,
        'Estado de Oxidación':'+1',
        'Densidad(g/cm3)':1.53,
    },
    'Sr':{
        'Grupo':'Alcalinoterreo',
        'Nombre':'Estroncio',
        'Número Atómico':38,
        'Peso Atómico':87.62,
        'Estado de Oxidación':'+2',
        'Densidad(g/cm3)':2.6,
    },
    'Y':{
        'Grupo':'Metal',
        'Nombre':'Itrio',
        'Número Atómico':39 ,
        'Peso Atómico':88.906,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':4.47
    },
    'Zr':{
        'Grupo':'Metal',
        'Nombre':'Circonio',
        'Número Atómico':40,
        'Peso Atómico':91.224,
        'Estado de Oxidación':4,
        'Densidad(g/cm3)':6.501
    },
    'Nb':{
        'Grupo':'Metal',
        'Nombre':'Niobio',
        'Número Atómico':41,
        'Peso Atómico':92.906,
        'Estado de Oxidación':'2-5',
        'Densidad(g/cm3)':8.57
    },
    'Mo':{
        'Grupo':'Metal',
        'Nombre':'Molibdeno',
        'Número Atómico':'42',
        'Peso Atómico':95.95,
        'Estado de Oxidación':'2-6',
        'Densidad(g/cm3)':10.28
    },
    'Tc':{
        'Grupo':'Metal',
        'Nombre':'Tecnecio',
        'Número Atómico':'43',
        'Peso Atómico':98,
        'Estado de Oxidación':'1-7',
        'Densidad(g/cm3)':11.5
    },
    'Ru':{
        'Grupo':'Metal',
        'Nombre':'Rutenio',
        'Número Atómico':44,
        'Peso Atómico':101.07,
        'Estados de Oxidación':'2-8',
        'Densidad(g/cm3)':12.37
    },
    'Rh':{
        'Grupo':'Metal',
        'Nombre':'Rodio',
        'Número Atómico':45,
        'Peso Atómico':102.91,
        'Estados de Oxidación':'(-1)-6',
        'Densidad(g/cm3)':12.45
    },
    'Pd':{
        'Grupo':'Metal',
        'Nombre':'Paladio',
        'Número Atómico':46,
        'Peso Atómico':106.42,
        'Estados de Oxidación':'0-6',
        'Densidad(g/cm3)':12.023
    },
    'Ag':{
        'Grupo':'Metal',
        'Nombre':'Plata',
        'Número Atómico':47,
        'Peso Atómico':107.87,
        'Estados de Oxidación':'1-4',
        'Densidad(g/cm3)':10.49
    },
    'Cd':{
        'Grupo':'Metal',
        'Nombre':'Cadmio',
        'Número Atómico':48,
        'Peso Atómico':112.41,
        'Estados de Oxidación':'1-2',
        'Densidad(g/cm3)':8.65
    },
    'In':{
        'Grupo':'Metal',
        'Nombre':'Indio',
        'Número Atómico':49,
        'Peso Atómico':114.82,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':7.31
    },
    'Sn':{
        'Grupo':'Metal',
        'Nombre':'Estaño',
        'Número Atómico':50,
        'Peso Atómico':118.71,
        'Estado de Oxidación':'2-4',
        'Densidad(g/cm3)':7.310
    },
    'Sb':{
        'Grupo':'Semimetal',
        'Nombre':'Antimonio',
        'Número Atómico':51,
        'Peso Atómico':121.76,
        'Estado de Oxidación':'3-5',
        'Densidad(g/cm3)':6.697
    },
    'Te':{
        'Grupo':'Semimetal',
        'Nombre':'Telurio',
        'Número Atómico':52,
        'Peso Atómico':127.60,
        'Estado de Oxidación':'2-6',
        'Densidad(g/cm3)':6.240
    },
    'I':{
        'Grupo':'No metal',
        'Nombre':'Yodo',
        'Número Atómico':53,
        'Peso Atómico':126.90,
        'Estado de Oxidación':'(-1)-7',
        'Densidad(g/cm3)':4.940
    },
    'Xe':{
        'Grupo':'Gas noble',
        'Nombre':'Xenon',
        'Número Atómico':54,
        'Peso Atómico':'131.29',
        'Estado de Oxidación':0,
        'Densidad(g/cm3)':0.0059
    },
    'Cs':{
        'Grupo':'Alcalino',
        'Nombre':'Cesio',
        'Número Atómico':55,
        'Peso Atómico':132.91,
        'Estado de Oxidación':1,
        'Densidad(g/cm3)':1.879
    },
    'Ba':{
        'Grupo':'Alcalinoterreo',
        'Nombre':'Bario',
        'Número Atómico':56,
        'Peso Atómico':137.33,
        'Estado de Oxidación':2,
        'Densidad(g/cm3)':3.510
    },
    'La':{
        'Grupo':'Lantanido',
        'Nombre':'Lantano',
        'Número Atómico':57,
        'Peso Atómico':138.91,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':6.146
    },
    'Ce':{
        'Grupo':'Lantanido',
        'Nombre':'Cerio',
        'Número Atómico':58,
        'Peso Atómico':140.12,
        'Estado de Oxidación':'3-4',
        'Densidad(g/cm3)':6.689
    },
    'Pr':{
        'Grupo':'Lantanido',
        'Nombre':'Praseodimio',
        'Número Atómico':59,
        'Peso Atómico':140.91,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':6.640
    },
    'Nd':{
        'Grupo':'Lantanido',
        'Nombre':'Neodimio',
        'Número Atómico':60,
        'Peso Atómico':144.24,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':7.010
    },
    'Pm':{
        'Grupo':'Lantanido',
        'Nombre':'Prometio',
        'Número Atómico':61,
        'Peso Atómico':145,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':7.264
    },
    'Sm':{
        'Grupo':'Lantanido',
        'Nombre':'Samario',
        'Número Atómico':62,
        'Peso Atómico':150.36,
        'Estado de Oxidación':'2-3',
        'Densidad(g/cm3)':7.353
    },
    'Eu':{
        'Grupo':'Lantanido',
        'Nombre':'Europio',
        'Número Atómico':63,
        'Peso Atómico':151.96,
        'Estado de Oxidación':'2-3',
        'Densidad(g/cm3)':5.244
    },
    'Gd':{
        'Grupo':'Lantanidos',
        'Nombre':'Gadolinio',
        'Número Atómico':64,
        'Peso Atómico':157.25,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':7.901
    },
    'Tb':{
        'Grupo':'Lantanido',
        'Nombre':'Terbio',
        'Número Atómico':65,
        'Peso Atómico':158.93,
        'Estado de Oxidación':4,
        'Densidad(g/cm3)':8.219
    },
    'Dy':{
        'Grupo':'Lantanido',
        'Nombre':'Disprosio',
        'Número Atómico':66,
        'Peso Atómico':162.50,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':8.551
    },
    'Ho':{
        'Grupo':'Lantanidos',
        'Nombre':'Holmio',
        'Número Atómico':67,
        'Peso Atómico':164.93,
        'Estado de Oxidación':'1-3',
        'Densidad(g/cm3)':9.795
    },
    'Er':{
        'Grupo':'Lantanido',
        'Nombre':'Erbio',
        'Número Atómico':68,
        'Peso Atómico':167.26,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':9.066
    },
    'Tm':{
        'Grupo':'Lantanidos',
        'Nombre':'Tulio',
        'Número Atómico':69,
        'Peso Atómico':168.93,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':9.320
    },
    'Yb':{
        'Grupo':'Lantanido',
        'Nombre':'Iterbio',
        'Número Atómico':70,
        'Peso Atómico':173.05,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':6.570
    },
    'Lu':{
        'Grupo':'Lantanido',
        'Nombre':'Lutecio',
        'Número Atómico':71,
        'Peso Atómico':174.97,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':9.841
    },
    'Hf':{
        'Grupo':'Metal',
        'Nombre':'Hafnio',
        'Número Atómico':72,
        'Peso Atómico':178.49,
        'Estado de Oxidación':'2-4',
        'Densidad(g/cm3)':13.310
    },
    'Ta':{
        'Grupo':'Metal',
        'Nombre':'Tántalo',
        'Número Atómico':73,
        'Peso Atómico':180.95,
        'Estado de Oxidación':'(-1)-5',
        'Densidad(g/cm3)':16.650
    },
    'W':{
        'Grupo':'Metal',
        'Nombre':'Wolframio',
        'Número Atómico':74,
        'Peso Atómico':183.84,
        'Estado de Oxidación':'2-6',
        'Densidad(g/cm3)':19.250
    },
    'Re':{
        'Grupo':'Metal',
        'Nombre':'Renio',
        'Número Atómico':75,
        'Peso Atómico':186.21,
        'Estado de Oxidación':'(-1)-7',
        'Densidad(g/cm3)':21.020
    },
    'Os':{
        'Grupo':'Metal',
        'Nombre':'Osmio',
        'Número Atómico':76,
        'Peso Atómico':190.23,
        'Estado de Oxidación':'(-2)-8',
        'Densidad(g/cm3)':22.59
    },
    'Ir':{
        'Grupo':'Metal',
        'Nombre':'Iridio',
        'Número Atómico':77,
        'Peso Atómico':192.22,
        'Estado de Oxidación':'(-3)-9',
        'Densidad(g/cm3)':22.56
    },
    'Pt':{
        'Grupo':'Metal',
        'Nombre':'Platino',
        'Número Atómico':78,
        'Peso Atómico':195.08,
        'Estado de Oxidación':'(-2)-6',
        'Densidad(g/cm3)':21.45
    },
    'Au':{
        'Grupo':'Metal',
        'Nombre':'Oro',
        'Numéro Atómico': 79,
        'Peso Atómico':196.97,
        'Estado de Oxidación':'1-3',
        'Densidad(g/cm3)':19.3
    },
    'Hg':{
        'Grupo':'Metal',
        'Nombre':'Mercurio',
        'Número Atómico': 80,
        'Peso Atómico':200.59,
        'Estado de Oxidación':'1-4',
        'Densidad(g/cm3)':13.534
    },
    'Tl':{
        'Grupo':'Metal',
        'Nombre':'Talio',
        'Número Atómico':81,
        'Peso Atómico':204.3833,
        'Estado de Oxidación':'1-3',
        'Densidad(g/cm3)':11.85
    },
    'Pb':{
        'Grupo':'Metal',
        'Nombre':'Plomo',
        'Número Atómico':82,
        'Peso Atómico':207.2,
        'Estado de Oxidación':'2-4',
        'Densidad(g/cm3)':11.34
    },
    'Bi':{
        'Grupo':'Metal',
        'Nombre':'Bismuto',
        'Número Atómico':83,
        'Peso Atómico':208.9803,
        'Estado de Oxidación':'3-5',
        'Densidad(g/cm3)':9.78
    },
    'Po':{
        'Grupo':'Metal',
        'Nombre':'Polonio',
        'Número Atómico':84,
        'Peso Atómico':208.9824,
        'Estado de Oxidación':'2-4',
        'Densidad(g/cm3)':9.19
    },
    'At':{
        'Grupo':'Semimetal',
        'Nombre':'Astato',
        'Número Atómico':85,
        'Peso Atómico':209.9871,
        'Estado de Oxidación':'1-3-5-7',
        'Densidad(g/cm3)':'-'
    },
    'Rn':{
        'Grupo':'Gas noble',
        'Nombre':'Radón',
        'Número Atómico':86,
        'Peso Atómico':222.0175,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':9.73
    },
    'Fr':{
        'Grupo':'Alcalino',
        'Nombre':'Francio',
        'Número Atómico':87,
        'Peso Atómico':223.0197,
        'Estado de Oxidación':1,
        'Densidad(g/cm3)':'-'
    },
    'Ra':{
        'Grupo':'Alcalinoterreo',
        'Nombre':'Radio',
        'Número Atómico':88,
        'Peso Atómico':226.0254,
        'Estado de Oxidación':2,
        'Densidad(g/cm3)':5.8
    },
    'Ac':{
        'Grupo':'Actinido',
        'Nombre':'Actinio',
        'Número Atómico':89,
        'Peso Atómico':227.0278,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':10.07
    },
    'Th':{
        'Grupo':'Actinido',
        'Nombre':'Torio',
        'Número Atómico':90,
        'Peso Atómico':232.0381,
        'Estado de Oxidación':4,
        'Densidad(g/cm3)':11.72
    },
    'Pa':{
        'Grupo':'Actinido',
        'Nombre':'Protactinio',
        'Número Atómico':91,
        'Peso Atómico':231.0358,
        'Estado de Oxidación':'4-5',
        'Densidad(g/cm3)':15.37
    },
    'U':{
        'Grupo':'Actinido',
        'Nombre':'Uranio',
        'Número Atómico':92,
        'Peso Atómico':238.0289,
        'Estado de Oxidación':'3-4-5-6',
        'Densidad(g/cm3)':19.05
    },
    'Np':{
        'Grupo':'Actinido',
        'Nombre':'Neptunio',
        'Número Atómico':93,
        'Peso Atómico':237,
        'Estado de Oxidación':'3-4-5-6',
        'Densidad(g/cm3)':20.45
    },
    'Pu':{
        'Grupo':'Actinido',
        'Nombre':'Plutonio',
        'Número Atómico':94,
        'Peso Atómico':244,
        'Estado de Oxidación':'3-4-5-6',
        'Densidad(g/cm3)':19.81
    },
    'Am':{
        'Grupo':'Actinido',
        'Nombre':'Americio',
        'Número Atómico':95,
        'Peso Atómico':243,
        'Estado de Oxidación':'3-4-5-6',
        'Densidad(g/cm3)':13.67
    },
    'Cm':{
        'Grupo':'Actinido',
        'Nombre':'Curio',
        'Número Atómico':96,
        'Peso Atómico':247,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':13.51
    },
    'Bk':{
        'Grupo':'Actinido',
        'Nombre':'Berkelio',
        'Número Atómico':97,
        'Peso Atómico':247,
        'Estado de Oxidación':'3-4',
        'Densidad(g/cm3)':14.78
    },
    'Cf':{
        'Grupo':'Actinido',
        'Nombre':'Californio',
        'Número Atómico':98,
        'Peso Atómico':251,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':'-'
    },
    'Es':{
        'Grupo':'Actinido',
        'Nombre':'Eistenio',
        'Número Atómico':99,
        'Peso Atómico':252,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':'-'
    },
    'Fm':{
        'Grupo':'Actinido',
        'Nombre':'Fermio',
        'Número Atómico':100,
        'Peso Atómico':257,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':'-'
    },
    'Md':{
        'Grupo':'Actinido',
        'Nombre':'Mendelevio',
        'Número Atómico':101,
        'Peso Atómico':258,
        'Estado de Oxidación':'2-3',
        'Densidad(g/cm3)':'-'
    },
    'No':{
        'Grupo':'Actinido',
        'Nombre':'Nodelio',
        'Número Atómico':102,
        'Peso Atómico':259,
        'Estado de Oxidación':'2-3',
        'Densidad(g/cm3)':'-'
    },
    'Lr':{
        'Grupo':'Actinido',
        'Nombre':'Laurencio',
        'Número Atómico':103,
        'Peso Atómico':262,
        'Estado de Oxidación':3,
        'Densidad(g/cm3)':'-'
    },
    'Rf':{
        'Grupo':'Metal',
        'Nombre':'Rutherfodio',
        'Número Atómico':104,
        'Peso Atómico':261.11,
        'Estado de Oxidación':4,
        'Densidad(g/cm3)':'-'
    },
    'Db':{
        'Grupo':'Metal',
        'Nombre':'Dubnio',
        'Número Atómico':105,
        'Peso Atómico':262.114,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Sg':{
        'Grupo':'Metal',
        'Nombre':'Seaborgio',
        'Número Atómico':106,
        'Peso Atómico':263.12,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Bh':{
        'Grupo':'Metal',
        'Nombre':'Bohrio',
        'Número Atómico':107,
        'Peso Atómico':262.12,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Hs':{
        'Grupo':'Metal',
        'Nombre':'Hassio',
        'Número Atómico':108,
        'Peso Atómico':264,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Mt':{
        'Grupo':'Desconocido',
        'Nombre':'Meitnerio',
        'Número Atómico':109,
        'Peso Atómico':266.1378,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Ds':{
        'Grupo':'Desconocido',
        'Nombre':'Darmstadtio',
        'Número Atómico':110,
        'Peso Atómico':269,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Rg':{
        'Grupo':'Desconocido',
        'Nombre':'Roentgenio',
        'Número Atómico':111,
        'Peso Atómico':272,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Cn':{
        'Grupo':'Desconocido',
        'Nombre':'Copernicio',
        'Número Atómico':112,
        'Peso Atómico':277,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uut':{
        'Grupo':'Desconocido',
        'Nombre':'Ununtrio',
        'Número Atómico':113,
        'Peso Atómico':284,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uuq':{
        'Grupo':'Desconocido',
        'Nombre':'Ununcuadio',
        'Número Atómico':114,
        'Peso Atómico':289,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uup':{
        'Grupo':'Desconocido',
        'Nombre':'Ununpentio',
        'Número Atómico':115,
        'Peso Atómico':288,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uuh':{
        'Grupo':'Desconocido',
        'Nombre':'Ununhexio',
        'Número Atómico':116,
        'Peso Atómico':289,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uus':{
        'Grupo':'Desconocido',
        'Nombre':'Ununseptio',
        'Número Atómico':117,
        'Peso Atómico':'-',
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uuo':{
        'Grupo':'Desconocido',
        'Nombre':'Ununoctio',
        'Número Atómico':118,
        'Peso Atómico':284,
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Uuu':{
        'Grupo':'Desconocido',
        'Nombre':'Ununenio',
        'Número Atómico':119,
        'Peso Atómico':'-',
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Ubn':{
        'Grupo':'Desconocido',
        'Nombre':'Unbinilio',
        'Número Atómico':120,
        'Peso Atómico':'-',
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
    'Ubu':{
        'Grupo':'Desconocido',
        'Nombre':'Unbiunio',
        'Número Atómico':121,
        'Peso Atómico':'-',
        'Estado de Oxidación':'-',
        'Densidad(g/cm3)':'-'
    },
}
nometales={'H','C','N','O','F','P','S','Cl','Se','Br','I'}
gasesnobles={'He','Ne','Ar','kr','Xe','Rn'}
semimetales={'B','Si','Ge','As','Sb','Te','At'}
metales={'Al','Ga','In','Sn','Tl','Pb','Bi','Po','Sc','Ti','V','Cr','Mn','Fe','Co','Ni','Cu','Y','Zr','Nb','Mo','Tc','Ru','Rh','Pd','Ag','Zn','Cd','Hf','Ta','W','Re','Os','Ir','Pt','Au','Hg','Rf','Db','Sg','Bh','Hs'}
lantanidos={'La','Ce','Pr','Nd','Pm','Sm','Eu','Gd','Tb','Dy','Ho','Er','Tm','Yb','Lu'}
actinidos={'Ac','Th','Pa','U','Np','Pu','Am','Cm','Bk','Cf','Es','Fm','Md','No','Lr'}
alcalinoterreos={'Be','Mg','Ca','Sr','Ba','Ra'}
alcalinos={'Li','Na','K','Rb','Cs','Fr'}
desconocidos={'Mt','Ds','Rg','Cn','Nh','Fl','Mc','Lv','Ts','Og','Uut','Uuq','Uup','Uuh','Uus','Uuo','Uue','Ubn','Ubu'}
print("Bienvenido a la Tabla Periódica de los Elementos!")
print("Ingrese el grupo de la Tabla Periopdica que desea investigar: ")
print("1.No metal\n2.Gas noble\n3.Semimetal\n4.Metal\n5.Lantanido\n6.Actinido\n7.Alcalinoterreo\n8.Alcalino\n9.Desconocido")
eleccion=input()
if eleccion=="1":
    print("los elementos que se encuentras en el grupo son:", nometales )
if eleccion=="2":
    print("los elementos que se encuentras en el grupo son:", gasesnobles )
if eleccion=="3":
    print("los elementos que se encuentras en el grupo son:", semimetales )
if eleccion=="4":
    print("los elementos que se encuentras en el grupo son:", metales )
if eleccion=="5":
    print("los elementos que se encuentras en el grupo son:", lantanidos )
if eleccion=="6.":
    print("los elementos que se encuentras en el grupo son:", actinidos )
if eleccion=="7":
    print("los elementos que se encuentras en el grupo son:", alcalinoterreos )
if eleccion=="8":
    print("los elementos que se encuentras en el grupo son:", alcalinos )
if eleccion=="9":
    print("los elementos que se encuentras en el grupo son:", desconocidos )
print("Ingrese por favor el símbolo del elemento químico que desee buscar: ")
for i in tabla_periodica:
    simbolo=input()
    if simbolo in tabla_periodica:
        print("La información del elemento químico",simbolo,"es",tabla_periodica[simbolo])
        tabla_periodicax={'H':1.00794,'He':4.002602,'Li':6.941,'Be':9.012182,'B':10.811,'C':12.011,'N':14.0067,'O':15.9994,'F':18.9984032,'Ne':20.1797,'Na':22.989768,'Mg':24.3050,'Al':26.981539,'Si':28.0855,'P':30.973762,'S':32.066,'Cl':35.4527,'Ar':39.948,'K':39.0983,'Ca':40.078,'Sc':44.955910,'Ti':47.88,'V':50.9415,'Cr':51.9961,'Mn':54.93805,'Fe':55.847,'Co':58.93320,'Ni':58.6934,'Cu':63.546,'Zn':65.39,'Ga':69.723,'Ge':72.61,'As':74.92159,'Se':78.96,'Br':79.904,'Kr':83.80,'Rb':85.4678,'Sr':87.62,'Y':88.90585,'Zr':91.224,'Nb':92.90638,'Mo':95.94,'Tc':97.9072,'Ru':101.07,'Rh':102.90550,'Pd':106.42,'Ag':107.8682,'Cd':112.441,'In':114.818,'Sn':118.710,'Sb':121.757,'Te':127.60,'I':126.90447,'Xe':131.29,'Cs':132.90543,'Ba':137.327,'La':138.9055,'Hf':178.49,'Ta':180.9479,'W':183.84,'Re':186.207,'Os':190.23,'Ir':192.22,'Pt':195.08,'Au':196.96654,'Hg':200.59,'Tl':204.3833,'Pb':207.2,'Bi':208.98037,'Po':208.9824,'At':209.9871,'Rn':222.0175,'Fr':223.0197,'Ra':226.0254,'Ac':227.0278,'Rf':261.11,'Db':262.114,'Sg':263.12,'Bh':262.12,'Hs':264,'Mt':266.1378,'Ds':269,'Rg':272,'Cn':277,'Uut':284,'Uuq':289,'Uup':288,'Uuh':289,'Uuo':284,'Ra':140.115,'Pr':140.90765,'Nd':144.24,'Pm':114.9127,'Sm':150.36,'Eu':151.965,'Gd':157.25,'Tb':158.92534,'Dy':162.50,'Ho':164.93032,'Tm':168.93421,'Lu':174.967,'Th':232.0381,'Pa':231.03588,'U':238.0289,'Np':237,'Pu':244,'Am':243,'Cm':247,'Bk':247,'Cf':251,'Es':252,'Fm':257,'Md':258,'No':259,'Lr':262}
        historial=[]
        print("Desea abrir la opción Calculadora de Elementos y Compuestos Químicos","\n","Si/No")
        while True:
            iniciar=input()
            if iniciar=="Si":
                print("1.¿Desea saber el Peso Atómico de un Elemento?\n2.¿Desea saber el Peso Atómico de un Compuesto?")
                eleccion=int(input("Elige una opción 1/2: "))
                if eleccion==1:
                    elemento=input("Ingrese el elemento químico: ")
                    print("El Peso Atómico de",elemento,"es",tabla_periodicax[elemento],"\n")
                    historial.append(tabla_periodicax[elemento])
                    print("¿Desea calcular otro elemento?\nSi/No\nVer historial")
                elif eleccion==2:
                    contador=0
                    sumatoria=0
                    compuestos=int(input("Ingrese la cantidad de elementos que va a tener su compuesto: "))
                    while (contador<compuestos):
                        print("Ingrese el",contador+1,"elemento químico: ")
                        elemento=input()
                        multiplicacion=float(input("Digite la cantidad por la cual lo va a multiplicar: "))
                        key=tabla_periodicax.get(elemento)
                        resultado=multiplicacion*key
                        print("El resultado de la multiplicación",multiplicacion,"es",resultado)
                        sumatoria=sumatoria+resultado
                        contador+=1
                        historial.append(sumatoria)
                        print(f"El Peso Atómico del compuesto es:{sumatoria}""\n")
                        print("¿Desea calcular otro compuesto?\nSi/No\nVer historial")
                elif iniciar=="Ver historial":
                    print(historial)
                    print("¿Desea calcular otro compuesto?\nSi/No")
                elif iniciar=="No":
                    print("Gracias por usar el programa, hasta pronto!")
                break
            else:
                print("Ingrese una opción valida para inicar el programa nuevamente\nSi/No\nVer historial")
